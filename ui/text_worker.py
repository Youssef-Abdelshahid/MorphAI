...
class TextAgentWorker:
    ...
    def _execute(self) -> None:
        ...
        self._sep()
        self._step("[1/9] Validating structured text file ...")
        file_errors = validate_text_file(self.data_path)
        if file_errors:
            for err in file_errors:
                self._log("ERROR", f"  {err}")
            self.q.put({"kind": "fail", "text": "Validation failed: " + "; ".join(file_errors)})
            return
