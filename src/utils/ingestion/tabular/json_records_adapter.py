...
from src.utils.ingestion.tabular.parsers import parse_json_records


class JsonRecordsTabularAdapter(BaseFormatAdapter):
    modality = "Tabular"
    input_format = "JSON / JSONL records"
    is_implemented = True
    format_key = "json_records"

    def to_internal_dataset(self, path: Path, record_path: str = "") -> AdapterResult:
        outcome = parse_json_records(Path(path), record_path=record_path)
        if not outcome.ok:
