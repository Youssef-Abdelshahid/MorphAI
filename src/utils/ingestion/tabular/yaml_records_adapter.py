...
from src.utils.ingestion.tabular.parsers import parse_yaml_records


class YamlRecordsTabularAdapter(BaseFormatAdapter):
    modality = "Tabular"
    input_format = "YAML records"
    is_implemented = True
    format_key = "yaml_records"

    def to_internal_dataset(self, path: Path, record_path: str = "") -> AdapterResult:
        outcome = parse_yaml_records(Path(path), record_path=record_path)
        if not outcome.ok:
